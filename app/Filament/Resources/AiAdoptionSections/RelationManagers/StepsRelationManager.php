...
class StepsRelationManager extends RelationManager
{
        ...
        return $schema
            ->components([
                ...
                TextInput::make('external_icon')
                    ...
                    ->maxLength(2048)
                    ->helperText('Example: images/icon-ai-detect.svg')
                    ->nullable(),
                TextInput::make('stat_emphasis')
                    ->label('Optional highlight (bold)')
                    ->maxLength(64)
                    ->helperText('e.g. “+32%”. Leave empty to hide the highlight row.')
                    ->nullable(),
                TextInput::make('stat_caption')
                    ->label('Optional highlight (caption)')
                    ->maxLength(120)
                    ->helperText('e.g. “Efficiency”. Shown next to the bold part.')
                    ->nullable(),
            ]);
    }
        ...
        return $table
            ...
            ->columns([
                ...
                ImageColumn::make('icon_path')
                    ...
                    ->getStateUsing(function (AiAdoptionStep $record): ?string {
    ...
    protected function hydrateIconPathForSave(array $data): array
    {
        ...
        if ($external !== '') {
            ...
            $data['icon_path'] = $file->store('ai-adoption-icons', 'webroot');
