...
namespace App\Filament\Resources\OfferSections\RelationManagers;
...
class OfferCardsRelationManager extends RelationManager
{
    protected static string $relationship = 'cards';

    protected static ?string $title = 'Offer cards';
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Select::make('theme')
                    ->options([
                        'gold' => 'Accent — soft gradient edge, solid CTA',
                        'blue' => 'Outlined — lighter card, outline CTA',
                    ])
                    ->default('gold')
                    ->required()
                    ->native(false),
                TextInput::make('pill_label')
                    ->required()
                    ->maxLength(120)
                    ->helperText('Small pill above the title (e.g. “Complimentary”).'),
                ...
                TextInput::make('external_icon')
                    ...
                    ->maxLength(2048)
                    ->helperText('Example: images/icon-free-assessment.svg')
                    ->nullable(),
                ...
                TextInput::make('cta_url')
                    ->label('Button URL')
                    ->maxLength(2048)
                    ->helperText('Use #contact or a full URL.')
                    ->nullable(),
            ]);
    }
        ...
        return $table
            ...
            ->columns([
                ...
                ImageColumn::make('icon_path')
                    ...
                    ->getStateUsing(function (OfferCard $record): ?string {
                        $url = $record->resolvedIconUrl();

                        return $url !== '' ? $url : null;
                    }),
                TextColumn::make('pill_label'),
    ...
    protected function hydrateIconPathForSave(array $data): array
    {
        ...
        if ($external !== '') {
            ...
            $data['icon_path'] = $file->store('offer-section-icons', 'webroot');
